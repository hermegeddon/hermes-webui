...
async function _savedSessionSidebarOnlyState(sid){
  ...
  if(cachedSession){
    const archived=!!cachedSession.archived;
    const running=!!(cachedSession&&(cachedSession.active_stream_id||cachedSession.pending_user_message||cachedSession.has_pending_user_message||cachedSession.is_streaming));
  ...
  try{
    ...
    if(data&&Object.prototype.hasOwnProperty.call(data,'session')&&session===null){
      ...
      return {sidebarOnly:false, archived:false};
    }
    const archived = !!(session&&session.archived);
    const running = !!(session&&(session.active_stream_id||session.pending_user_message||session.has_pending_user_message||session.is_streaming));
    return {sidebarOnly:archived||running, archived};
  }catch(e){
    return null;
  }
